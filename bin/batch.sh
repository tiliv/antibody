...
if command -v parallel >/dev/null 2>&1; then
  ...
  parallel -j "$CPU" --will-cite $script
  exit
fi
...
if command -v gxargs >/dev/null 2>&1; then
  ...
  gxargs -P "$CPU" -n 1 $script
  exit
fi

# 3) System xargs: probe for -P support (macOS BSD xargs supports -P)
if xargs -P 1 echo </dev/null >/dev/null 2>&1; then
  echo "xargs -P $CPU -n 1 $script"
  xargs -P "$CPU" -n 1 $script
  exit
fi

# 4) Last resort: sequential (portable)
